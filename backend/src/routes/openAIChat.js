  ...
  try {
    ...
    const openaiResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "사용자의 입력 문장에서 장소 검색에 사용할 주요 키워드(명사)를 추출해 HTML 형식으로 반환해 주세요. 이전 대화를 참고해서 맥락을 유지하며, 예: '서울에서 맛있는 한식을 먹고 싶어요' -> {\"keywords\": [\"서울\", \"한식\"]}, '부산도 추천해줘' -> {\"keywords\": [\"부산\", \"한식\"]}",
        },
        ...sessionData[userId],
      ],
      temperature: 0.5,
  ...
  try {
    ...
    const openaiResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "사용자의 요청에 따라 여행 계획, 동선, 추천 여행지 정보, 날씨에 맞는 액티비티 등을 제안해 주세요. 응답은 간결하고 핵심적인 문장으로 3~4문장 내외로 작성해 주세요. 이전 대화를 참고해서 맥락을 유지하며, 자연스럽고 유용한 응답을 제공하세요.",
        },
        ...sessionData[userId],
      ],
      temperature: 0.7,
